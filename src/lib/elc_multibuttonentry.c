...
_format_count(int count, void *data EINA_UNUSED)
{
   ...
   //TIZEN_ONLY(20150429): To support mobile UX.
...
_item_del(Elm_Multibuttonentry_Item_Data *item)
{
   ...
   */
   ...
   evas_object_smart_callback_call(obj, SIG_ITEM_DELETED, EO_OBJ(item));
...
//TIZEN_ONLY(20150824): cleared selection on unfocus
static void
_on_item_unfocus(void *data,
                 Evas_Object *obj EINA_UNUSED,
                 void *event_info EINA_UNUSED)
{
   Elm_Multibuttonentry_Item_Data *it = data;
   if (!it) return;
   elm_layout_signal_emit(VIEW(it), "elm,state,default", "elm");
}
//
                 ...
                 void *data)
{
   ...
   //TIZEN_ONLY(20150720): layout click signal change.
   ...
   //TIZEN_ONLY(20150824): "unfocused" callback added
   evas_object_smart_callback_add
      (VIEW(item), "unfocused", _on_item_unfocus, item);
   //
   evas_object_show(VIEW(item));
           ...
           const char *str)
{
   ...
   if (!strlen(str))
     ...
     {
        //TIZEN_ONLY(20150803): Makes label packed only one time.
        if (sd->label_packed)
          elm_box_unpack(sd->box, sd->label);
        //
...
_view_init(Evas_Object *obj, Elm_Multibuttonentry_Data *sd)
{
   ...
   //TIZEN_ONLY(20150429): Entry property set for mobile UX.
