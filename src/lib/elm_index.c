...
_omit_calc(void *data, int num_of_items, int max_num_of_items)
{
   ...
   EINA_LIST_FREE(sd->omit, o)
      sd->omit = eina_list_remove(sd->omit, o);

   EINA_LIST_FOREACH(sd->items, l, it)
     {
        if (it->omitted)
          EINA_LIST_FREE(it->omitted, it2)
             it->omitted = eina_list_remove(it->omitted, it2);
   ...
   Elm_Index_Omit **t = (Elm_Index_Omit **)malloc(sizeof(Elm_Index_Omit *) * max_group_num);
   ...
   for (i = 0; i < max_num_of_items; i++)
     {
        if (omit_info[i] > 1)
          {
             t[g] = (Elm_Index_Omit *)malloc(sizeof(Elm_Index_Omit));
             t[g]->offset = sum;
             t[g]->count = omit_info[i];
             sd->omit = eina_list_append(sd->omit, t[g]);
             g++;
          }
        sum += omit_info[i];
     }
