           ...
           Evas_Coord h)
{
   ...
   DBG("_obj_place obj = 0x%x, %d, %d, %d, %d", obj, x, y, w, h);
...
_overlay_default_show(Overlay_Default *ovl)
{
   ...
   if (show_engine_obj)//to display engine object the overlay must NOT belong to class
    ...
    if (disp && EINA_FALSE == show_engine_obj)
     {
...
double min_lon __UNUSED__, double max_lat __UNUSED__, double min_lat __UNUSED__, int animation __UNUSED__)
{
...
/*
  if (sd->engine->route_add)
        ret = sd->engine->route_add(ELM_WIDGET_DATA(sd)->obj, route->data);
   ...
   if (!ret)
     {
      ...
      route->engine_handle = ret;//store the map object handle created from engine
*/
...
_map_custom_route_del(Elm_Map_Route *route)
{
   ...
   //if (sd->engine->route_del)
   //    sd->engine->route_del(ELM_WIDGET_DATA(sd)->obj, route->data);
...
elm_map_overlay_class_add(Evas_Object *obj)
{
  ...
  //ELM_MAP_ENG_OBJECT_CREATE(sd->engine->group_create, overlay->engine_obj, ELM_WIDGET_DATA(overlay->wsd)->obj, ovl->lon, ovl->lat);
   ...
   if (overlay->type == ELM_MAP_OVERLAY_TYPE_DEFAULT)
     {
         ...
         if (overlay->engine_obj)
            ELM_MAP_ENG_OBJECT_SET(overlay->wsd->engine->object_visibility, ELM_WIDGET_DATA(overlay->wsd)->obj, overlay->engine_obj, 0);
     }
   ...
   if (overlay->type == ELM_MAP_OVERLAY_TYPE_DEFAULT)
     {
        ...
        _overlay_default_class_content_update(overlay->ovl, NULL);
        if (overlay->engine_obj)
           ELM_MAP_ENG_OBJECT_SET(overlay->wsd->engine->object_visibility, ELM_WIDGET_DATA(overlay->wsd)->obj, overlay->engine_obj, 1);
     }
                              ...
                              const Elm_Map_Overlay *parent)
{
   ...
   if (strcmp(bubble->wsd->engine->name, INTERNAL_ENGINE_NAME))
