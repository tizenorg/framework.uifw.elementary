...
static void
_format_parse(char *str, char *fmt)
{
   ...
   if (sd->inc_button)
     {
        ...
        elm_widget_style_set(sd->inc_button, eina_strbuf_string_steal(buf));
     }
   if (sd->text_button)
     {
        eina_strbuf_append_printf(buf, "spinner/%s", elm_widget_style_get(obj));
        elm_widget_style_set(sd->text_button, eina_strbuf_string_steal(buf));
     }
   if (sd->dec_button)
     {
        eina_strbuf_append_printf(buf, "spinner/decrease/%s", elm_widget_style_get(obj));
        elm_widget_style_set(sd->dec_button, eina_strbuf_string_steal(buf));
     }
   if (_elm_config->access_mode)
     _access_spinner_register(obj, EINA_TRUE);
...
static int
_decimal_points_get(char *label)
{
   char fmt[32];
   int decimal_point= 0;
