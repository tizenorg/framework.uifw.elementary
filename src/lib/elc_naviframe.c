...
static Elm_Naviframe_Mod_Api *
_nf_mod_init(void)
{
   Elm_Module *mod;
   initted++;
   ERR("_nf_mod_init initted %d", initted);
   if (initted > 1) return;
   ...
   if (prev_it)
     {
        ...
        if (nf_mod && nf_mod->tizen_pop_effect) nf_mod->tizen_pop_effect(prev_it);
#endif
     }
   elm_object_signal_emit(VIEW(it), "elm,state,cur,popped,deferred", "elm");
   edje_object_message_signal_process(elm_layout_edje_get(VIEW(it)));

#ifndef ELM_FEATURE_WEARABLE_RECTANGLE
   // TIZEN ONLY
   if (nf_mod && nf_mod->tizen_pop_effect) nf_mod->tizen_pop_effect(it);
#endif
...
#ifdef ELM_FEATURE_WEARABLE_RECTANGLE
        if (nf_mod && nf_mod->tizen_push_effect) nf_mod->tizen_push_effect(prev_it);
#endif
     }
...
#ifndef ELM_FEATURE_WEARABLE_RECTANGLE
   // TIZEN ONLY
   if (nf_mod && nf_mod->tizen_push_effect) nf_mod->tizen_push_effect(it);
#endif
   return ECORE_CALLBACK_CANCEL;
}
...
#ifdef ELM_FEATURE_WEARABLE_CIRCLE
