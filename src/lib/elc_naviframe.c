...
_access_info_cb(void *data, Evas_Object *obj EINA_UNUSED)
{
   ...
   const char *title = N_("Title");
   const char *subtitle = N_("Subtitle");
   char *ret;
   ...
   layout = VIEW(nit);
   title_text = elm_object_part_text_get(layout, TITLE_PART);
   if (!title_text) return NULL;

   buf = eina_strbuf_new();
   eina_strbuf_append_printf(buf, "%s, %s", title_text, title);

   subtitle_text = elm_object_part_text_get(layout, SUBTITLE_PART);
   if(subtitle_text && strlen(subtitle_text) > 0) {
                       ...
                       const char *source EINA_UNUSED)
{
   ...
   if (sd->freeze_events)
     evas_object_freeze_events_set(VIEW(it), EINA_FALSE);
//Tizen Only(20150817) : Prevent item pop while push animation is in progress.
...
EOLIAN static Evas_Object *
_elm_naviframe_elm_interface_atspi_component_accessible_at_point_get(Eo *obj, Elm_Naviframe_Data *_pd EINA_UNUSED, Eina_Bool screen_coords, int x, int y)
{
   Elm_Object_Item *eo_item;
   Eo *view;
   Eina_Bool contains;
   ...
   if (!view || !eo_isa(view, ELM_INTERFACE_ATSPI_COMPONENT_MIXIN))
     return NULL;

   eo_do(view, contains = elm_interface_atspi_component_contains(screen_coords, x, y));
   return contains ? view : NULL;
}
