...
_access_info_cb(void *data, Evas_Object *obj __UNUSED__)
{
   ...
   edje = (Evas_Object *)data;
   info = edje_object_part_text_get(edje, "elm.text.title");
   if (!info) return NULL;

   buf = eina_strbuf_new();
   eina_strbuf_append(buf, info);

   info = edje_object_part_text_get(edje, "elm.text.subtitle");
   if (!info) goto end;
        ...
        if (!_access_object_get(it, TITLE_ACCESS_PART))
          {
             ao =_elm_access_edje_object_part_object_register
                     (WIDGET(it), VIEW(it), TITLE_ACCESS_PART);
