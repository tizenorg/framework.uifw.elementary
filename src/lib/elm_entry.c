   ...
   if (cy + y - adjh + oy < 0)
     {
        ...
        adj_scale = (double)edj_content_h / ch;
...
_access_info_cb(void *data, Evas_Object *obj __UNUSED__)
{
   ...
   if ((sd->password && _elm_config->access_password_read_enable) ||
   ...
   else if (sd->password && !(_elm_config->access_password_read_enable))
     {
        ...
        eina_strbuf_append_printf(buf, E_("%d characters"), evas_string_char_len_get(text));
...
_access_state_cb(void *data, Evas_Object *obj __UNUSED__)
{
   ...
   if (elm_widget_disabled_get(data))
     eina_strbuf_append(buf, E_("Disabled"));
   ...
   if (!sd->editable)
   ...
   else if (!elm_widget_disabled_get(data))
     {
        if ((elm_object_focus_get(data) && (elm_win_keyboard_mode_get(top) == ELM_WIN_KEYBOARD_OFF) && !sd->on_activate) ||
          ...
          eina_strbuf_append(buf, E_("Editing"));
     }
                  ...
                  if (pos > 0)
                    {
                       ...
                       _elm_access_say(data, E_("Cursor at beginning of text"), EINA_FALSE);
                    }
                  else
                    {
                       elm_entry_cursor_end_set(data);
                       _elm_access_say(data, E_("Cursor at end of text"), EINA_FALSE);
                    }
               }
...
elm_entry_add(Evas_Object *parent)
{
   ...
   //TODO: Need to check why clipping is not happening when adding hit_rect as smart member of obj
   //evas_object_smart_member_add(sd->hit_rect, obj);
   evas_object_smart_member_add(sd->hit_rect, sd->entry_edje);
