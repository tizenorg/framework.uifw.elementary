   ...
   if (sd->mgf_proxy)
     {
        ...
        evas_object_color_set(sd->mgf_proxy, 255, 255, 255, 0);
        evas_object_hide(sd->mgf_proxy);
...
static void
_magnifier_content_resize(void *data)
{
   ELM_ENTRY_DATA_GET(data, sd);
   ...
   if (elm_widget_focus_get(obj))
     {
        ...
        _cnpinit(obj, NULL, NULL); // TIZEN ONLY
...
// TIZEN ONLY
static Eina_Bool
_elm_entry_smart_event(Evas_Object *obj,
               ...
               void *event_info)
{
   ...
   if (ev->buttons == 1)
     {
        if ((sd->long_pressed) && (sd->magnifier_enabled))
          {
             _magnifier_show(data);
             _magnifier_move(data);
          }
   ...
   if (!_elm_config->desktop_entry)
     {
        ...
        if (sd->magnifier_showing)
          _magnifier_content_resize(data);
     }
   //
}
...
_elm_entry_smart_add(Evas_Object *obj)
{
   ...
   edje_object_signal_callback_add
      (priv->entry_edje, "long,pressed", "elm.text",
...
_elm_entry_smart_set_user(Elm_Entry_Smart_Class *sc)
{
   ...
   //TIZEN ONLY
   ELM_WIDGET_CLASS(sc)->event = _elm_entry_smart_event;
   //
