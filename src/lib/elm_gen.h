...
struct _Elm_Gen_Item_Class
{
   ...
   const char   *decorate_item_style; /**< Style used if item is set to a decorate mode. NULL if you don't care. currently it's used only in genlist. */
...
typedef enum
{
   ELM_GLOB_MATCH_NO_ESCAPE = (1 << 0), /**< @internal Treat backslash as an ordinary character instead of escape */
   ELM_GLOB_MATCH_PATH = (1 << 1), /**< @internal Match a slash in string only with a slash in pattern and not by an asterisk (*) or a question mark (?) metacharacter, nor by a bracket expression ([]) containing a slash. */
   ELM_GLOB_MATCH_PERIOD = (1 << 2), /**< @internal Leading  period in string has to be matched exactly by a period in pattern. A period is considered to be leading if it is the first character in string, or if both ELM_GLOB_MATCH_PATH is set and the period immediately follows a slash. */
   ELM_GLOB_MATCH_NOCASE = (1 << 3) /**< @internal The pattern is matched case-insensitively. */
} Elm_Glob_Match_Flags; /**< @internal Glob matching bitfiled flags. @since 1.11 */
