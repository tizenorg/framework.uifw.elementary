      ...
      const char *source __UNUSED__)
{
   ...
   else if (layout_mod->timepicker_layout)
     {
   ...
   for (idx = 0; idx < DATETIME_FIELD_COUNT -1; idx++)
     {
        ...
        if ((obj != entry) && elm_object_focus_get(entry))
          {
             elm_layout_signal_emit(layout_mod->layout_field[idx],
   ...
   for (idx = 0; idx <= ELM_DATETIME_DATE; idx++)
     {
       ...
       snprintf(buf, sizeof(buf), "datetime_layout_date/%s", field_styles[idx]);
       ...
       evas_object_smart_callback_add(entry, "clicked", _entry_clicked_cb, layout_mod);
   ...
   for (idx = ELM_DATETIME_HOUR; idx < ELM_DATETIME_AMPM; idx++)
     {
       ...
       snprintf(buf, sizeof(buf), "datetime_layout_time/%s", field_styles[idx]);
       elm_object_style_set(entry, buf);
       elm_entry_magnifier_disabled_set(entry, EINA_TRUE);
       elm_entry_context_menu_disabled_set(entry, EINA_TRUE);
       elm_entry_markup_filter_append(entry, elm_entry_filter_accept_set, &digits_filter_data);
   ...
   for (idx = 0; idx <= ELM_DATETIME_DATE; idx++)
     {
        ...
        elm_object_style_set(spinner, "vertical_date_picker");
...
obj_focus_hook(Elm_Datetime_Module_Data *module_data __UNUSED__)
{
   ...
   layout_mod = (Layout_Module_Data *)module_data;
   if (!layout_mod || layout_mod->popup) return;

   if (elm_widget_focus_get(obj))
     evas_object_show(layout_mod->popup);
   else
     evas_object_hide(layout_mod->popup);
#endif
}
