...
_hour_validity_checking_filter(void *data, Evas_Object *obj, char **text)
{
   ...
   button_mod = (Button_Module_Data *)data;
   if (!button_mod) goto error;

   insert = *text;
   curr_str = elm_object_text_get(obj);
   if (!curr_str) goto error;

   len = strlen(curr_str);
   if (len < min_text_len) goto error;
   ...
   if(field_type == ELM_DATETIME_YEAR)
     {
        if (_string_to_integer(elm_object_text_get(button_mod->datetime_entry_field[field_type])) == 0)
          _datetime_entry_value_set(button_mod, field_type, 1902 - STRUCT_TM_YEAR_BASE_VALUE);
        ...
        if (curr_time.tm_hour < STRUCT_TM_TIME_12HRS_MAX_VALUE)
          {
             ...
             elm_object_domain_translatable_text_set(button_mod->datetime_entry_field[field_type], PACKAGE, E_("AM"));
          }
        else
          {
             elm_object_domain_translatable_text_set(obj, PACKAGE, E_("PM"));
             elm_object_domain_translatable_text_set(button_mod->datetime_entry_field[field_type], PACKAGE, E_("PM"));
          }
     }
...
_field_access_register(Button_Module_Data *button_mod, Evas_Object *obj, Elm_Datetime_Field_Type field_type)
{
   ...
   _elm_access_text_set(_elm_access_object_get(ao), ELM_ACCESS_INFO, buf);
}
...
access_register(Elm_Datetime_Module_Data *module_data)
{
   const char* type = NULL;
   Button_Module_Data *button_mod;
   Elm_Access_Info *ai_top_foot;
   int idx;
   char increase_buf[BUFF_SIZE];
   char decrease_buf[BUFF_SIZE];
   ...
   for (idx = 0; idx < DATETIME_FIELD_COUNT; idx++)
     {
        switch (idx)
          {
             case ELM_DATETIME_YEAR:
               snprintf(increase_buf, sizeof(increase_buf), "%s", E_("IDS_COM_BODY_INCREASE_YEAR"));
               snprintf(decrease_buf, sizeof(decrease_buf), "%s", E_("IDS_COM_BODY_DECREASE_YEAR"));
               break;

             case ELM_DATETIME_MONTH:
               snprintf(increase_buf, sizeof(increase_buf), "%s", E_("IDS_COM_BODY_INCREASE_MONTH"));
               snprintf(decrease_buf, sizeof(decrease_buf), "%s", E_("IDS_COM_BODY_DECREASE_MONTH"));
               break;

             case ELM_DATETIME_DATE:
               snprintf(increase_buf, sizeof(increase_buf), "%s", E_("IDS_COM_BODY_INCREASE_DAY"));
               snprintf(decrease_buf, sizeof(decrease_buf), "%s", E_("IDS_COM_BODY_DECREASE_DAY"));
               break;

             case ELM_DATETIME_HOUR:
               snprintf(increase_buf, sizeof(increase_buf), "%s", E_("IDS_COM_BODY_INCREASE_HOUR"));
               snprintf(decrease_buf, sizeof(decrease_buf), "%s", E_("IDS_COM_BODY_DECREASE_HOUR"));
               break;

             case ELM_DATETIME_MINUTE:
               snprintf(increase_buf, sizeof(increase_buf), "%s", E_("IDS_COM_BODY_INCREASE_MINUTE"));
               snprintf(decrease_buf, sizeof(decrease_buf), "%s", E_("IDS_COM_BODY_DECREASE_MINUTE"));
               break;
        ...
        ai_top_foot = _elm_access_object_get(button_mod->top_button[idx]);
        _elm_access_text_set(ai_top_foot , ELM_ACCESS_TYPE, increase_buf);
        _elm_access_text_set(ai_top_foot , ELM_ACCESS_CONTEXT_INFO, type);
        _elm_access_callback_set(ai_top_foot, ELM_ACCESS_INFO, _access_info_top_btn_cb, button_mod);


        ai_top_foot = _elm_access_object_get(button_mod->foot_button[idx]);
        _elm_access_text_set(ai_top_foot , ELM_ACCESS_TYPE, decrease_buf);
        _elm_access_text_set(ai_top_foot , ELM_ACCESS_CONTEXT_INFO, type);
        _elm_access_callback_set(ai_top_foot, ELM_ACCESS_INFO, _access_info_foot_btn_cb, button_mod);
        ...
        switch (idx)
          {
             ...
             case ELM_DATETIME_AMPM:
                snprintf(buf, sizeof(buf), "datetime/style%d", 3);
                evas_object_event_callback_add(field_obj, EVAS_CALLBACK_MOUSE_UP, _am_pm_released_cb, (void *)button_mod);
                break;
          }
