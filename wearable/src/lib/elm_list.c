...
static Eina_Bool _select_timer(void *data);
            ...
            (!sd->multi))
     {
        if (type == EVAS_CALLBACK_KEY_DOWN && !sd->key_down)
        ...
        else if (type == EVAS_CALLBACK_KEY_UP && sd->key_down)
          {
             ...
             it->highlight_timer = ecore_timer_add(ITEM_SELECT_TIMER, _select_timer, it);
             sd->key_down = NULL;
             ev->event_flags |= EVAS_EVENT_FLAG_ON_HOLD;
             return EINA_TRUE;
          }
...
_elm_list_smart_on_focus(Evas_Object *obj)
{
   ...
   if (elm_widget_focus_get(obj))
   ...
   else
     {
        ...
        // and focus is not on list, call select_timer forcely
        if (sd->key_down)
          {
             ...
             void *event_info)
{
   ...
   if (evas_event_down_count_get(evas) != 0)
     {
        if (!it->highlight_timer) _item_unhighlight(it);
     }
   if (it->long_timer)
     {
        ecore_timer_del(it->long_timer);
