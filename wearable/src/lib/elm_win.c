   ...
   if (ch_aux_hint)
     {
        ...
        EINA_LIST_FOREACH(aux_hints, l, id)
          {
             evas_object_smart_callback_call(obj, SIG_AUX_HINT_ALLOWED, (void *)(long)id);
          }
                          ...
                          void *event_info __UNUSED__)
{
   ...
   if (sd->focus_highlight.prev.target == obj)
        ...
        if ((unsigned int)e->data.l[0] == sd->x.xwin)
          {
             if ((unsigned int)e->data.l[1] ==
               ...
               {
                  _elm_access_highlight_cycle(ELM_WIDGET_DATA(sd)->obj, ELM_FOCUS_NEXT);
               }
             else if ((unsigned int)e->data.l[1] ==
                      ECORE_X_ATOM_E_ILLUME_ACCESS_ACTION_READ_PREV)
               {
                  _elm_access_highlight_cycle(ELM_WIDGET_DATA(sd)->obj, ELM_FOCUS_PREVIOUS);
               }
             else if ((unsigned int)e->data.l[1] ==
                         ...
                         void *event)
{
   ...
   if (e->atom == ECORE_X_ATOM_E_WINDOW_DESKTOP_LAYOUT)
     {
        ret = ecore_x_window_prop_card32_get(e->win,
                  ...
                  void *event)
{
   ...
   if (!_elm_widget_top_win_focused_get(data))
     return ECORE_CALLBACK_PASS_ON;
