      ...
      Evas_Object *parent, Evas_Coord *xoff, Evas_Coord *yoff)
{
   ...
   Evas_Object *icon;
   const char *text;
   Evas_Coord sel_x = 0, sel_y = 0, sel_h = 0, sel_w = 0, tb_w = 0;
   ELM_ENTRY_DATA_GET(data, sd);
   ...
   if (edje_object_part_text_selection_lines_number_get(sd->entry_edje, "elm.text") > 1)
     {
        Evas_Coord x;
        edje_object_part_text_selection_line_properties_get(sd->entry_edje, "elm.text", 0, &x, NULL, NULL, NULL);
        char *new_text = malloc(strlen(text)+30); // text + tabstops
        sprintf(new_text, "<+tabstops=%d><\\t></>%s", x, text);
        evas_object_textblock_text_markup_set(icon, new_text);
        free(new_text);
     }
   else
   ...
   evas_object_geometry_get(elm_entry_textblock_get(data), NULL, NULL, &tb_w, NULL);

   evas_object_move(icon, sel_x, sel_y);
   evas_object_resize(icon, tb_w, sel_h);
...
_magnifier_proxy_update(Evas_Object *obj, double scale)
{
   ...
   evas_object_geometry_get(sd->mgf_proxy, &mx, &my, &mw, &mh);
   ...
   px = -(((double) cx / dw) * (scale * dw)) + ((double) mw * 0.5) - ctnx * scale;
   py = -(((double) cy / dh) * (scale * dh)) + ((double) mh * 0.5) - ctny * scale;
   pw = (dw * scale);
   ph = (dh * scale);
...
_magnifier_move(void *data)
{
   ...
   if (sd->mgf_type == _ENTRY_MAGNIFIER_FIXEDSIZE)
     evas_object_move(sd->mgf_bg,
                      cx + x - (mw / 2) + ctnx, cy + y - adjh + oy + ctny);
...
_magnifier_create(void *data)
{
   ...
   if (sd->mgf_type == _ENTRY_MAGNIFIER_FILLWIDTH)
     evas_object_resize(sd->mgf_bg, w, sd->mgf_height);
   ...
   if (sd->dnd_enabled)
     {
        ...
        if (!sd->hit_selection)
        ...
        else
          {
             ...
             if ((sel) && (sel[0]))
               {
                  sd->dnd_started = EINA_TRUE;
                  elm_cnp_selection_set(data, ELM_SEL_TYPE_PRIMARY,
                                        ELM_SEL_FORMAT_MARKUP, sel, strlen(sel));
                  elm_drag_start(data, ELM_SEL_FORMAT_TARGETS,
                                 NULL,
...
_viewport_region_get(Evas_Object *data)
{
   ...
   if (sd->scroll)
     {
        evas_object_geometry_get(sd->scr_edje,
                                 &geometry.x,
                                 &geometry.y,
                                 &geometry.w,
                                 &geometry.h);
   ...
   if (elm_widget_focus_get(obj))
     {
        ...
        edje_object_signal_emit(sd->entry_edje, "elm,action,focus", "elm");
        sd->mgf_type = _ENTRY_MAGNIFIER_FIXEDSIZE; // TIZEN ONLY
...
_long_press_cb(void *data)
{
   ...
   if (!_elm_config->desktop_entry && !sd->magnifier_enabled) // TIZEN ONLY
     _menu_call(data);
   ...
   if (ev->button == 1)
     {
        ...
        if (sd->magnifier_enabled)
          {
             _magnifier_hide(data);
             if ((sd->long_pressed) && (!sd->dnd_started))
               _menu_call(data);
          }
   ...
   if (ev->buttons == 1)
     {
        if (sd->long_pressed)
          {
             if ((!sd->dnd_started) && (sd->magnifier_enabled))
               {
                  _magnifier_show(data);
                  _magnifier_move(data);
               }
              ...
              Elm_Selection_Data *drop)
{
   ...
   if (drop->data) elm_entry_entry_insert(obj, drop->data);
   edje_object_part_text_cursor_copy
...
elm_entry_add(Evas_Object *parent)
{
   ...
   elm_drop_target_add
     (obj, ELM_SEL_FORMAT_MARKUP | ELM_SEL_FORMAT_IMAGE, _dnd_enter_cb, NULL, _dnd_leave_cb, NULL,
     _dnd_pos_cb, obj, _drag_drop_cb, NULL);
...
////////////// TIZEN ONLY APIs
EAPI void
elm_entry_dnd_enabled_set(Evas_Object *obj, Eina_Bool enabled)
{
   ELM_ENTRY_CHECK(obj);
   ELM_ENTRY_DATA_GET(obj, sd);
   sd->dnd_enabled = enabled;
}
