   ...
   EINA_LIST_REVERSE_FOREACH(top_objects_list, itr, top_obj)
     {
        ...
        while (object)
          {
             dropable = evas_object_data_get(object, "dropable");
             if (dropable)
                goto end;
             else
                         ...
                         Elm_Drop_Cb dropcb, void *cbdata)
{
   ...
   evas_object_data_set(obj, "dropable", drop);
...
_x11_elm_drop_target_del(Evas_Object *obj)
{
   ...
   dropable = evas_object_data_get(obj, "dropable");
   if (dropable)
     {
        drops = eina_list_remove(drops, dropable);
        evas_object_data_del(obj, "dropable");
        free(dropable);
        dropable = NULL;
     }
