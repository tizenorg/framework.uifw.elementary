...
void
elm_mapbuf_point_color_set(Evas_Object *obj, int idx, int r, int g, int b, int a)
{
   ELM_MAPBUF_CHECK(obj);
   ELM_MAPBUF_DATA_GET(obj, sd);

   if ((idx < 0) || (idx >= (sizeof(sd->colors)/sizeof(sd->colors[0]))))
     {
   ...
   sd->colors[idx].r = (unsigned char) r;
   sd->colors[idx].g = (unsigned char) g;
   sd->colors[idx].b = (unsigned char) b;
   sd->colors[idx].a = (unsigned char) a;
