...
static void
_parent_highlight(Evas_Object *obj)
{
   ELM_WIDGET_DATA_GET(obj, sd);

   Evas_Object *o = elm_widget_parent_get(obj);
   if (o) _parent_highlight(o);
        ...
        if (ho)
          {
             ...
             if ((nsp) && (sp != nsp))
               {
                  _scroll_repeat_events_set(sp, EINA_TRUE);
                  _scroll_repeat_events_set(nsp, EINA_FALSE);
      ...
      case 0:
         ...
         if (!ho)
         ...
         else
           {
              ...
              s_parents = _access_scrollable_parents_get(ho, &sx, &sy, &sw, &sh);
              if (!s_parents) return;
              ...
              sp = eina_list_data_get(s_parents);
              _scroll_repeat_events_set(sp, EINA_FALSE);
         ...
         evas_event_feed_mouse_move(evas, x - offset.x, y - offset.y, ecore_loop_time_get() * 1000, NULL);
...
void
_elm_access_highlight_read_enable_set(Evas_Object *obj, Eina_Bool enabled)
{
   Evas *evas;
   Eina_List *l = NULL;
   Evas_Object *o, *sp;
...
_elm_access_highlight_cycle(Evas_Object *obj, Elm_Focus_Direction dir)
{
   ...
   if (!ho) elm_widget_focus_cycle(obj, dir);
   else if (!_access_action_callback_call(ho, type, NULL))
     {
        ...
        if (type == ELM_ACCESS_ACTION_HIGHLIGHT_NEXT)
          _highlight_next_get_internal(ho, ELM_HIGHLIGHT_DIR_NEXT, &target);
        else
          _highlight_next_get_internal(ho, ELM_HIGHLIGHT_DIR_PREVIOUS, &target);
...
EAPI void
_elm_access_say(Evas_Object *obj, const char *txt)
{
   if (!_elm_config->access_mode) return;
   if (!highlight_read_enable) return;
   ...
   if (win)
     {
        ...
        if (win_type == ELM_WIN_SOCKET_IMAGE || win_type == -1)
   ...
   switch (type)
     {
      ...
      case ELM_ACCESS_ACTION_ENABLE:
        _elm_access_highlight_read_enable_set(obj, EINA_TRUE);
        break;

      case ELM_ACCESS_ACTION_DISABLE:
        _elm_access_highlight_read_enable_set(obj, EINA_FALSE);
        break;
