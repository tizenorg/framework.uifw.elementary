...
elm_player_add(Evas_Object *parent)
{
   ...
   sd->forward = _player_button_add(obj, "media_player/forward", _forward);
   sd->info = _player_button_add(obj, "media_player/info", _info);
   sd->next = _player_button_add(obj, "media_player/next", _next);
   sd->pause = _player_button_add(obj, "media_player/pause", _pause);
   sd->play = _player_button_add(obj, "media_player/play", _play);
   sd->prev = _player_button_add(obj, "media_player/prev", _prev);
   sd->rewind = _player_button_add(obj, "media_player/rewind", _rewind);
   sd->stop = _player_button_add(obj, "media_player/stop", _stop);

   sd->slider = elm_slider_add(obj);
   elm_slider_indicator_format_function_set
     (sd->slider, _double_to_time, _str_free);
   elm_slider_units_format_function_set
     (sd->slider, _double_to_time, _str_free);
   elm_slider_min_max_set(sd->slider, 0, 0);
   elm_slider_value_set(sd->slider, 0);
   elm_object_disabled_set(sd->slider, EINA_TRUE);
   evas_object_size_hint_align_set(sd->slider, EVAS_HINT_FILL, 0.5);
   evas_object_size_hint_weight_set
     (sd->slider, EVAS_HINT_EXPAND, EVAS_HINT_EXPAND);

   elm_layout_content_set(obj, "media_player/slider", sd->slider);
   evas_object_smart_callback_add
     (sd->slider, "changed", _update_position, obj);
