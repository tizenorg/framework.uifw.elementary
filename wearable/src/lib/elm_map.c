   ...
   if (ovl->icon)
   ...
   else
     {
        if (!strcmp(ovl->wsd->engine->name, INTERNAL_ENGINE_NAME))
             ...
             evas_object_color_set
                   (ovl->layout, 0, 0, 0, 0);//hide this completely
        elm_layout_theme_set
                    ...
                    void *data)
{
   ...
   // FIXME: typo: perpective -> perspective
   perspective_cb = eina_module_symbol_get(m, "map_module_perpective_set");
...
_map_capture_clean(Evas_Object *obj)
{
  ...
  EINA_LIST_FOREACH_SAFE(sd->icons_map_capture, l, ll, iconhandle)
        ELM_MAP_ENG_OBJECT_DELETE(sd->engine->icon_remove,  ELM_WIDGET_DATA(sd)->obj, iconhandle);
                  ...
                  void *data)
{
  ...
  if (!strcmp(sd->engine->name, INTERNAL_ENGINE_NAME)) return ret;

 /*add all the icons using the nokia engine api
*/
  if (sd->engine->icon_add && sd->engine->icon_remove)
    {
  ...
  ret = sd->engine->map_capture(ELM_WIDGET_DATA(sd)->obj, w, h, _map_capture_cb);
  if (!ret)
      _map_capture_clean(obj);
