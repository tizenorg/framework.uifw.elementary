...
_access_shrink_info_cb(void *data, Evas_Object *obj __UNUSED__)
{
   ...
   if (num)
     {
        snprintf(str, sizeof(str), E_("And %d more"), num);
        return strdup(str);
     }
   return NULL;
}
...
static void
_on_layout_focus(void *data, Evas_Object *obj __UNUSED__, void *event_info __UNUSED__)
{
...
static void
_on_layout_unfocus(void *data, Evas_Object *obj __UNUSED__, void *event_info __UNUSED__)
{
   Elm_Multibuttonentry_Item *it = (Elm_Multibuttonentry_Item *)data;
...
_item_disable_hook(Elm_Object_Item *it)
{
   ...
   if (elm_widget_item_disabled_get(it))
     elm_layout_signal_emit(VIEW(mbe_it), "disabled", "");
   else
     elm_layout_signal_emit(VIEW(mbe_it), "default", "");
          ...
          const void *data)
{
   ...
   evas_object_smart_callback_add
      (VIEW(it), "focused", _on_layout_focus, it);
   evas_object_smart_callback_add
      (VIEW(it), "unfocused", _on_layout_unfocus, it);
   ...
   else // 3rd delete
     {
        ...
        if (!trans)
          _item_deleting_effect_add
             ((Elm_Multibuttonentry_Item *)sd->selected_item);
#else
...
elm_multibuttonentry_add(Evas_Object *parent)
{
   ...
   _elm_access_text_set
      (_elm_access_object_get(obj), ELM_ACCESS_TYPE, E_("Double tap to edit the recipients"));
