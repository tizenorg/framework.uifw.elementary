...
static Eina_Bool
_elm_thumb_smart_on_focus(Evas_Object *obj)
{
   if (!ELM_WIDGET_CLASS(_elm_thumb_parent_sc)->on_focus(obj)) return EINA_FALSE;
...
_elm_thumb_smart_set_user(Elm_Thumb_Smart_Class *sc)
{
   ...
   ELM_WIDGET_CLASS(sc)->on_focus = _elm_thumb_smart_on_focus;
}
