   ...
   if (sd->state)
     {
        ...
        if (_elm_config->access_mode != ELM_ACCESS_MODE_OFF)
          if (!(strcmp(elm_widget_style_get(obj),"on&off")))
            _elm_access_say(obj,E_("On"));
          else
            _elm_access_say(obj, E_("Tick")); //TIZEN ONLY (2013.12.04): TTS UI
     }
   else
     {
        ...
        if (_elm_config->access_mode != ELM_ACCESS_MODE_OFF)
            if (!(strcmp(elm_widget_style_get(obj),"on&off")))
              _elm_access_say(obj,E_("Off"));
            else
              _elm_access_say(obj, E_("Untick")); //TIZEN ONLY (2013.12.04): TTS UI
     }
