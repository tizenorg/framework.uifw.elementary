...
#define DEL_ANIM_CNT_MAX 5 // frame time
#define ADD_ANIM_CNT_MAX 5 // frame time
...
_item_cache_push(Elm_Gen_Item *it)
{
   ...
   if (eina_list_data_find_list(sd->add_fx.items, it) ||
       (eina_list_data_find_list(sd->add_fx.items, it)))
     {
              ...
              Eina_Bool calc)
{
   ...
   if (_focus_enabled(WIDGET(it)) && (it == sd->focused))
     {
   ...
   if (GL_IT(it)->scrl_y < oy)
     {
        ...
        sd->s_iface->content_region_show
           (ELM_WIDGET_DATA(sd)->obj, sd->pan_x,
            sd->pan_y - REORDER_FASTER, ow, oh);
     }
   ...
   if (sd->add_fx.cnt <= 0)
     {
        ...
        sd->add_fx.cnt = ADD_ANIM_CNT_MAX;;
        sd->add_fx.anim = NULL;
        return ECORE_CALLBACK_CANCEL;
     }
   return ECORE_CALLBACK_RENEW;
...
_add_fx_space_get(Elm_Gen_Item *it)
{
   ...
   size = size * ((double)sd->add_fx.cnt/ADD_ANIM_CNT_MAX);
   return size;
}
   ...
   if (sd->del_fx.cnt <= 0)
     {
        ...
        sd->del_fx.cnt = DEL_ANIM_CNT_MAX;
        sd->del_fx.anim = NULL;
        return ECORE_CALLBACK_CANCEL;
     }
   return ECORE_CALLBACK_RENEW;
...
_del_fx_space_get(Elm_Gen_Item *it)
{
   ...
   size = size * (1 - ((double)sd->del_fx.cnt/DEL_ANIM_CNT_MAX));
   return size;
}
...
_elm_genlist_pan_smart_calculate(Evas_Object *obj)
{
   ...
   if (psd->wsd->comp_y)
     {
        psd->wsd->s_iface->content_region_show
           (ELM_WIDGET_DATA(psd->wsd)->obj, psd->wsd->pan_x,
   ...
   if (psd->wsd->add_fx.anim)
     {
        ...
        int alpha = 255 * (1 - ((double)psd->wsd->add_fx.cnt/ADD_ANIM_CNT_MAX));
        EINA_LIST_FOREACH(psd->wsd->add_fx.items, l, it)
          {
   ...
   if (psd->wsd->del_fx.anim)
     {
        ...
        int alpha = 255 * ((double)psd->wsd->del_fx.cnt/DEL_ANIM_CNT_MAX);
        EINA_LIST_FOREACH(psd->wsd->del_fx.items, l, it)
          {
             if (it->deco_all_view)
                evas_object_color_set(it->deco_all_view, alpha, alpha, alpha, alpha);
...
static void _item_unfocused(Elm_Gen_Item *it)
{
   ...
   edje_object_signal_emit
      (VIEW(sd->focused), SIGNAL_UNFOCUSED, "elm");
   if (sd->focused->deco_all_view)
      edje_object_signal_emit
         (sd->focused->deco_all_view, SIGNAL_UNFOCUSED, "elm");
...
static void _item_focused(Elm_Gen_Item *it, Elm_Genlist_Item_Scrollto_Type type)
{
   ...
   if (sd->focused != it) _item_unfocused(sd->focused);
...
_item_select(Elm_Gen_Item *it)
{
   ...
   edje_object_signal_emit
	   (VIEW(sd->focused), SIGNAL_CLICKED, "elm");
   evas_object_ref(obj);
...
static Eina_Bool _item_focus_next(Elm_Genlist_Smart_Data *sd, Focus_Dir dir)
{
   ...
   if (dir == FOCUS_DIR_DOWN || dir == FOCUS_DIR_UP)
   ...
   else if (dir == FOCUS_DIR_LEFT || dir == FOCUS_DIR_RIGHT)
     {
        ...
        if (sd->focused->flipped)
          contents = GL_IT(sd->focused)->flip_content_objs;
        else
          {
             contents = sd->focused->content_objs;
             if (sd->focused->deco_all_view)
               {
                  if (contents)
                    contents = GL_IT(sd->focused)->deco_all_contents;
                  else
                    contents = eina_list_merge(contents,
                                  GL_IT(sd->focused)->deco_all_contents);
               }
          }
   ...
   if ((!strcmp(ev->keyname, "Left")) ||
            ...
            !strcmp(ev->keyname, "KP_Enter"))
     {
        ...
        if (type == EVAS_CALLBACK_KEY_DOWN && !sd->key_down)
             ...
             if (sd->focused)
               {
                  ...
                  sd->key_down->long_timer = ecore_timer_add
          ...
          {
             edje_object_signal_emit
                (VIEW(sd->key_down), SIGNAL_FOCUSED, "elm");
             if (sd->key_down->deco_all_view)
                 edje_object_signal_emit
                    (sd->key_down->deco_all_view, SIGNAL_FOCUSED, "elm");

             if (sd->key_down->long_timer)
               ecore_timer_del(sd->key_down->long_timer);
             sd->key_down->long_timer = NULL;
             if (GL_IT(sd->key_down)->highlight_timer)
               ecore_timer_del(GL_IT(sd->key_down)->highlight_timer);
             GL_IT(sd->key_down)->highlight_timer = ecore_timer_add
...
{
   ELM_GENLIST_DATA_GET(obj, sd);
   if (sd->focused) _item_focused(sd->focused, ELM_GENLIST_ITEM_SCROLLTO_IN);
}
        ...
        if (_focus_enabled(obj))
          {
             if (sd->focused)
               {
                  if (!sd->focused_content)
                    _item_focused(sd->focused, ELM_GENLIST_ITEM_SCROLLTO_IN);
                  else
                   {
                      Eina_List *l = sd->focused->content_objs;
                      // focused content is not a member of focused item,
                      // focus set on focused_content
                      if (!(eina_list_data_find(l, sd->focused_content)) &&
                          !(eina_list_data_find(l, elm_widget_parent_get
                                               (sd->focused_content))))
                  ...
                  void *event_info)
{
   ...
   // FIXME: When scroller freezed, event flag change to none.
   // if (ev->event_flags & EVAS_EVENT_FLAG_ON_HOLD)
   if (it->dragging)
     {
...
elm_genlist_add(Evas_Object *parent)
{
   ...
   elm_gesture_layer_cb_set
      (sd->g_layer, ELM_GESTURE_N_LINES, ELM_GESTURE_STATE_START,
       _gesture_n_lines_start_cb, sd);
   elm_gesture_layer_cb_set
      (sd->g_layer, ELM_GESTURE_N_LINES, ELM_GESTURE_STATE_MOVE,
       _gesture_n_lines_cb, sd);

   elm_gesture_layer_cb_set
      (sd->g_layer, ELM_GESTURE_N_FLICKS, ELM_GESTURE_STATE_START,
       _gesture_n_lines_start_cb, sd);
   elm_gesture_layer_cb_set
      (sd->g_layer, ELM_GESTURE_N_FLICKS, ELM_GESTURE_STATE_MOVE,
       _gesture_n_flicks_cb, sd);
...
_item_free(Elm_Gen_Item *it)
{
   ...
   if (VIEW(it)) evas_object_color_set(VIEW(it), 255, 255, 255, 255);
   ...
   if (it == sd->key_down) sd->key_down = NULL;
...
elm_genlist_items_count(const Evas_Object *obj)
{
   ...
   return sd->item_count - eina_list_count(sd->del_fx.items)
                 - eina_list_count(sd->del_fx.pending_items);
}
