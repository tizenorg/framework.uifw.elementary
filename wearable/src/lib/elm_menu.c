...
elm_menu_add(Evas_Object *parent)
{
   ...
   sd->location = elm_icon_add(obj);

   sd->hv = elm_hover_add(obj);
   elm_widget_mirrored_set(sd->hv, EINA_FALSE);

   elm_object_style_set(sd->hv, "menu");
   evas_object_smart_callback_add(sd->hv, "clicked", _hover_clicked_cb, obj);

   sd->bx = elm_box_add(obj);
   elm_widget_mirrored_set(sd->bx, EINA_FALSE);
   evas_object_size_hint_weight_set
     (sd->bx, EVAS_HINT_EXPAND, EVAS_HINT_EXPAND);
