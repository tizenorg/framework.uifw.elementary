...
//
   ...
   if (sid->loop_h && spx > (mx + (ww / 2)))
     spx = 0;
   if (sid->loop_v && spy > (my + (wh / 2)))
     spy = 0;
             ...
             if (x == minx)
               {
                  ...
                  if (!sid->tizen_freeze_effect)
                    edje_object_signal_emit(sid->edje_obj, "elm,edge,left",
                                            "elm");
               }
             if (x == (mx + minx))
               {
                  ...
                  if (!sid->tizen_freeze_effect)
                    edje_object_signal_emit(sid->edje_obj, "elm,edge,right",
                                            "elm");
               }
          }
        if (y != py)
                               ...
                               Evas_Coord h)
{
   ...
   if (_elm_scroll_content_region_show_internal(obj, &x, &y, w, h))
     {
        sid->tizen_freeze_effect = EINA_TRUE;
        _elm_scroll_content_pos_set(obj, x, y, EINA_FALSE);
        sid->tizen_freeze_effect = EINA_FALSE;
        sid->down.sx = x;
        sid->down.sy = y;
   ...
   if (dt >= 0.0)
     {
        ...
        if (!_elm_config->thumbscroll_bounce_enable || !sid->bounce_horiz)
          {
             if ((x <= minx) && (sid->down.dx >= 0)) no_bounce_x_end = EINA_TRUE;
             if (!sid->loop_h && ((x - minx) >= maxx) && (sid->down.dx <= 0)) no_bounce_x_end = EINA_TRUE;
          }
        if (!_elm_config->thumbscroll_bounce_enable || !sid->bounce_vert)
          {
             if ((y <= miny) && (sid->down.dy >= 0)) no_bounce_y_end = EINA_TRUE;
             if (!sid->loop_v && ((y - miny) >= maxy) && (sid->down.dy <= 0)) no_bounce_y_end = EINA_TRUE;
          }
        if ((dt >= 1.0) ||
   ...
   if (sid->pagerel_v > 0.0)
     {
        if (sid->pagecount_v != 0)
          h = ch / (sid->pagecount_v * sid->pagerel_h);
                  ...
                  if (at > 0)
                    {
                       ...
                       if ((_elm_config->thumbscroll_friction > 0.0) &&
                           (vel > _elm_config->thumbscroll_momentum_threshold))
                         {
                            ...
                            if (aw  > vw * 3)
                              {
                                 if (sid->down.dx > 0) sid->down.dx = vw * 3;
                                 else sid->down.dx = -(vw * 3);
                              }
                            ah = abs(sid->down.dy);
                            if (ah  > vh * 3)
                              {
                                 if (sid->down.dy > 0) sid->down.dy = vh * 3;
                                 else sid->down.dy = -(vh * 3);
                              }
...
#define ELM_MOVE_PER_SECOND (47.5)
//>Time between two subsequent events
#define ELM_MOVE_TIMEOUT (1./ELM_MOVE_PER_SECOND)
...
static Eina_Bool _elm_scroll_get_pos(Elm_Scrollable_Smart_Interface_Data *sid,
...
#define QUEUE_SIZE 3 /* for event queue size */
   ...
   source = ecore_animator_source_get();
...
//TIZEN: Temporary code begin for media controller issue.
static void
_elm_scroll_page_show(Evas_Object *obj,
                      int pagenumber_h,
                      int pagenumber_v);
struct tizen_pageshow
{
   Evas_Object *obj;
   int pagenumber_h;
};
...
static Eina_Bool
tizen_page_show_animator_cb(void *data)
{
   struct tizen_pageshow *t = data;
   _elm_scroll_page_show(t->obj, t->pagenumber_h, 0);
   return EINA_FALSE;
}

static void
_tizen_for_mediacontroller(Evas_Object *obj, int pagenumber_h)
{
   static struct tizen_pageshow t;
   Evas_Coord w, h;

   t.obj = obj;
   t.pagenumber_h = pagenumber_h;

   evas_object_geometry_get(obj, NULL, NULL, &w, &h);

   if ((w == 0) && (h == 0))
     ecore_animator_add(tizen_page_show_animator_cb, &t);
}
//TIZEN: Temporary code end for media controller issue.
                      ...
                      int pagenumber_v)
{
   ...
   //FIXME: temporary code. should be removed.
   _tizen_for_mediacontroller(obj, pagenumber_h);
