...
_scroller_size_calc(Evas_Object *obj)
{
   ...
   edje_object_size_max_get(sd->layout, NULL, &lh);
   sd->max_sc_h = lh;
}
...
_elm_popup_smart_del(Evas_Object *obj)
{
   ...
   ELM_SAFE_FREE(notify, evas_object_del);
        ...
        if (!sd->button_count)
          {
             ...
             _elm_access_text_set(_elm_access_object_get(ao),
                                  ELM_ACCESS_TYPE,
                                  E_("A central popup is open, double tap to close it"));
...
//   if (elm_widget_parent_get(sd->notify) != obj)
   if (!strcmp(elm_widget_style_get(obj), "default"))
   ...
   if (sd->items)
     {
        ...
        evas_object_size_hint_max_get(sd->scr, &maxw, &maxh);
     }
...
_list_add(Evas_Object *obj)
{
   ...
   if (wsd->orient_mode == 90 || wsd->orient_mode == 270)
     elm_widget_theme_object_set(obj, sd->layout, "popup", "layout/landscape", "default");
   else
     elm_widget_theme_object_set(obj, sd->layout, "popup", "layout", "default");
...
static Eina_Bool
_elm_popup_smart_on_focus(Evas_Object *obj)
{
   Elm_Popup_Item *item;
   ELM_POPUP_DATA_GET(obj, sd);

   if (!ELM_WIDGET_CLASS(_elm_popup_parent_sc)->on_focus(obj))
     return EINA_FALSE;
   ...
   if (sd->items)
     {
        EINA_LIST_FOREACH(sd->items, elist, item)
        {
           if (rotation == 90 || rotation == 270)
             {
                elm_widget_theme_object_set(popup, sd->layout, "popup", "layout/landscape", "default");
                elm_widget_theme_object_set
                   (WIDGET(item), VIEW(item), "popup", "item/landscape", elm_widget_style_get(WIDGET(item)));
             }
           else
             {
                elm_widget_theme_object_set(popup, sd->layout, "popup", "layout", "default");
                elm_widget_theme_object_set
                   (WIDGET(item), VIEW(item), "popup", "item", elm_widget_style_get(WIDGET(item)));
             }
           _item_update(item);
...
elm_popup_add(Evas_Object *parent)
{
   ...
   evas_object_size_hint_weight_set
     (ELM_WIDGET_DATA(sd)->resize_obj, EVAS_HINT_EXPAND, EVAS_HINT_EXPAND);
   evas_object_size_hint_align_set
     (ELM_WIDGET_DATA(sd)->resize_obj, EVAS_HINT_FILL, EVAS_HINT_FILL);

   //FIXME: No idea. just keep the theme_set() here so that popup theme inherited by parent work fine.
