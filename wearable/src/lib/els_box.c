...
_els_box_layout(Evas_Object *o, Evas_Object_Box_Data *priv, int horizontal, int homogeneous, int rtl)
{
   ...
   double dxx, dyy; /* for storing children's location */
   const Eina_List *l;
   Evas_Object *obj;
        ...
        if (!_elm_config->box_gap_fill)
        ...
        else
          {
             if (horizontal)
               {
                  double ww, hh, ow, oh;
                  Evas_Coord cww; /* for storing calculated width */

                  if (homogeneous)
                    {
                       ww = ((double)w / (double)count);
                    }
                  else
                    {
                       ww = mnw;
                       if ((expand > 0) && (xw))
                         {
                            ow = (double)((w - minw) * wx) / expand;
                            ww += ow;
                         }
                    }
                  cww = (int)round(dxx + ww) - (int)round(dxx);
                  hh = h;
                  ow = mnw;
                  if (fw) ow = cww;
                  if ((mxw >= 0) && (mxw < ow)) ow = mxw;
                  ...
                  evas_object_move(obj,
                                   ((!rtl) ? (xx) : (x + (w - (xx - x) - cww)))
                                   + (Evas_Coord)(((cww - ow)) * ax),
                                   yy + (Evas_Coord)(((hh - oh)) * ay));
                  evas_object_resize(obj, ow, oh);
                  xx += cww;
                  dxx += ww;
                  xx += priv->pad.h;
               ...
               {
                  double ww, hh, ow, oh;
                  Evas_Coord chh; /* for storing calculated height */

                  if (homogeneous)
                    {
                       hh = ((double)h / (double)count);
                    }
                  else
                    {
                       hh = mnh;
                       if ((expand > 0) && (xh))
                         {
                            oh = (double)((h - minh) * wy) / expand;
                            hh += oh;
                         }
                    }
                  chh = (int)round(dyy + hh) - (int)round(dyy);
                  ww = w;
                  ow = mnw;
                  if (fw) ow = ww;
                  if ((mxw >= 0) && (mxw < ow)) ow = mxw;
                  ...
                  evas_object_move(obj,
                                   xx + (Evas_Coord)(((double)(ww - ow)) * ax),
                                   yy + (Evas_Coord)(((double)(chh - oh)) * ay));
                  evas_object_resize(obj, ow, oh);
                  yy += chh;
                  dyy += hh;
                  yy += priv->pad.v;
