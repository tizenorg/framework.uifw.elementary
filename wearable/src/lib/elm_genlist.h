...
/**
 * Set whether a given genlist item is hidden or not
 *
 * @param it The item
 * @param  Use @c EINA_TRUE, to make it hide, @c
 * EINA_FALSE to make it show
 *
 * This sets the hidden state of an item.
 *
 * @ingroup Genlist
 */
EAPI void
...
/**
 * Get the genlist item's hidden mode
 *
 * @param obj The genlist object
...
/**
 * Set the genlist's realization mode
 *
 * @param obj The genlist object
 * @param mode The realization mode
 * (EINA_TRUE = on, EINA_FALSE = off)
 *
 * Defaulty, genlist disable realiztion mode and genlist realize and
 * unrealize some items when it is needed. If realization mode is on,
 * all items are realized when genlist is created and no items are unrealized.
 * If this mode is on, if content's size is changed, items size will be
 * changed accordingly. Defaulty, genlist does not change items' sizes
 * eventhough those can be changed for the performance reason.
 * It will consumes more memory and can decrease performance. So if application
 * append many items, do not use the realization mode.
 *
 * @ingroup Genlist
 */
EAPI void
...
/*
 * Get the genlist's realization mode
 *
 * @param obj The genlist object
