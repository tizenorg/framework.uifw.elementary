...
static Eina_Bool
_spinner_double_click_timer_cb(void *data)
{
...
static void
_spinner_double_click_cb(void *data,
                         Evas_Object *obj __UNUSED__,
                         const char *emission __UNUSED__,
                         const char *source __UNUSED__)
{
...
static char *
_access_date_picker_info_cb(void *data, Evas_Object *obj __UNUSED__)
{
   char buf[BUFF_SIZE];
   int curr = (int*)data;
   switch(curr)
     {
        case ELM_DATETIME_YEAR:
          sprintf(buf, E_("flick up and down to adjust %s or double tap to open keyboard"), E_("year"));
          return strdup(buf);
        case ELM_DATETIME_MONTH:
          sprintf(buf, E_("flick up and down to adjust %s or double tap to open keyboard"), E_("month"));
          return strdup(buf);
        case ELM_DATETIME_DATE:
          sprintf(buf, E_("flick up and down to adjust %s or double tap to open keyboard"), E_("date"));
          return strdup(buf);
        default:
          break;
      }
...
static char *
_access_time_picker_info_cb(void *data, Evas_Object *obj __UNUSED__)
{
	char buf[BUFF_SIZE];
   int curr = (int*)data;
   switch(curr)
     {
       case ELM_DATETIME_HOUR:
          sprintf(buf, E_("flick up and down to adjust %s or double tap to open keyboard"), E_("hour"));
          return strdup(buf);
       case ELM_DATETIME_MINUTE:
          sprintf(buf, E_("flick up and down to adjust %s or double tap to open keyboard"), E_("minute"));
          return strdup(buf);
       default:
          break;
     }
...
{
   Evas_Object *spinner, *entry, *ao, *ai;
   char buf[BUFF_SIZE];
   int idx, loc;
   ...
   for (idx = 0; idx <= ELM_DATETIME_DATE; idx++)
     {
        ...
        ao = _elm_access_edje_object_part_object_register(spinner, elm_layout_edje_get(spinner), "access");
        ai = _elm_access_object_get(ao);
        _elm_access_text_set(ai, ELM_ACCESS_TYPE, E_("spinner"));
        _elm_access_callback_set(ai, ELM_ACCESS_INFO, _access_info_cb, spinner);
        _elm_access_callback_set(ai, ELM_ACCESS_STATE, _access_date_picker_info_cb, idx);
        _elm_access_activate_callback_set(ai, _access_activate_spinner_cb, spinner);
        ...
        edje_object_signal_callback_add(elm_layout_edje_get(spinner),
                    "mouse,down,1,double", "*", _spinner_double_click_cb, popup_mod);
...
static char *
_access_ampm_info_cb(void *data, Evas_Object *obj __UNUSED__)
{
   return strdup(E_("Double tap to change"));
}

static void
_create_timepicker_layout(Popup_Module_Data *popup_mod)
{
   Evas_Object *spinner, *ampm, *entry, *ao, *ai;
   char buf[BUFF_SIZE];
   int idx;
   ...
   for (idx = ELM_DATETIME_HOUR; idx < ELM_DATETIME_AMPM; idx++)
     {
       ...
       _elm_access_text_set(ai, ELM_ACCESS_TYPE, E_("spinner"));
       _elm_access_callback_set(ai, ELM_ACCESS_INFO, _access_info_cb, spinner);
       _elm_access_callback_set(ai, ELM_ACCESS_STATE, _access_time_picker_info_cb, idx);
       _elm_access_activate_callback_set(ai, _access_activate_spinner_cb, spinner);
       elm_spinner_step_set(spinner, 1);
       elm_spinner_wrap_set(spinner, EINA_TRUE);
       elm_spinner_label_format_set(spinner, "%02.0f");
                 ...
                 const char *source __UNUSED__)
{
   ...
   for (idx = 0; idx <= ELM_DATETIME_DATE; idx++)
     {
       case 0:
          sprintf(buf, "%d %s", (STRUCT_TM_YEAR_BASE_VALUE + set_date->tm_year), E_("year"));
          return strdup(buf);
       case 1:
          sprintf(buf, "%d %s", set_date->tm_mon+1, E_("month"));
          return strdup(buf);
       case 2:
          sprintf(buf, "%d %s", set_date->tm_mday, E_("date"));
          return strdup(buf);
       default:
          break;
     }
...
_access_set(Evas_Object *obj, Elm_Datetime_Field_Type field_type)
{
   ...
   eina_strbuf_append_printf(buf,
                             "%d %s, %d %s",
                             set_time.tm_hour, E_("hour"),
                             set_time.tm_min, E_("minute"));
   if (popup_mod->time_12hr_fmt)
     {
        if (popup_mod->is_pm)
