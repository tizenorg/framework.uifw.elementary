...
_ctxpopup_position(Evas_Object *obj __UNUSED__)
{
   ...
   Evas_Coord sx, sy, sw, sh;
   Evas_Coord x, y, w, h;
   Elm_Ctxpopup_Direction ctx_dir;
   int gap = 13; //in GUI
   ...
   if (!edje_object_part_text_selection_geometry_get(ext_mod->ent, "elm.text", &sx, &sy, &sw, &sh))
     { //cannot get selection shape
        ...
        ctx_dir = elm_ctxpopup_direction_get(ext_mod->popup);
        if (ctx_dir == ELM_CTXPOPUP_DIRECTION_DOWN)
          {
             ...
             evas_object_move(ext_mod->popup, x, y);
             ctx_dir = elm_ctxpopup_direction_get(ext_mod->popup);
             if (ctx_dir == ELM_CTXPOPUP_DIRECTION_UP)
        ...
        if (ch_visible)
          {
             if ((chy < cy) && (ctx_dir == ELM_CTXPOPUP_DIRECTION_UP))
               {
                  y = ey + chy;
                  evas_object_move(ext_mod->popup, x, y);
                  ctx_dir = elm_ctxpopup_direction_get(ext_mod->popup);
             ...
             else if ((chy > cy) && (ctx_dir == ELM_CTXPOPUP_DIRECTION_DOWN))
               {
                  y = ey + chy + chh;
                  evas_object_move(ext_mod->popup, x, y);
                  ctx_dir = elm_ctxpopup_direction_get(ext_mod->popup);
        ...
        if (((ctx_dir != ELM_CTXPOPUP_DIRECTION_UP) &&
             (ctx_dir != ELM_CTXPOPUP_DIRECTION_DOWN)) || need_update)
          {
             y = ey + cy + ch / 2;
             elm_ctxpopup_direction_priority_set(ext_mod->popup,
                                                 ELM_CTXPOPUP_DIRECTION_LEFT,
...
_translate_menu(void *data __UNUSED__, Evas_Object *obj, void *event_info __UNUSED__)
{
   ...
   int setval = appsvc_set_operation(b, APPSVC_OPERATION_PICK);
   setval = appsvc_set_appid(b, "com.samsung.stranslator-shared");
